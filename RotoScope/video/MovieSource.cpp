...
bool CMovieSource::TryOpen(const char* p_filename)
{
	...
	if (FAILED(m_graphBuilder.CoCreateInstance(CLSID_FilterGraph, NULL, CLSCTX_INPROC))) return Error(TEXT("Unable to create a DirectShow filter graph"));
	...
	if (FAILED(m_videoGrabberBF.CoCreateInstance(CLSID_SampleGrabber))) return Error(TEXT("Unable to create a DirectShow sample grabber"));
	...
	if (FAILED(m_graphBuilder->AddFilter(m_videoGrabberBF, L"VideoGrabber"))) return Error(TEXT("Unable to add the video sample grabber to the filter graph"));

	//
	// Create the audio sample grabber
	//

	if (FAILED(m_audioGrabberBF.CoCreateInstance(CLSID_SampleGrabber))) return Error(TEXT("Unable to create a DirectShow sample grabber"));
	...
	if (FAILED(m_graphBuilder->AddFilter(m_audioGrabberBF, L"AudioGrabber"))) return Error(TEXT("Unable to add the video sample grabber to the filter graph"));
	...
	if (hr != S_OK) return Error(TEXT("Unable to open video file"));

	// Determine if anything connected to our two sample grabbers.
	if (InputConnected(m_videoGrabberBF)) m_hasvideo = true;
	else m_videoGrabberBF.Release(); // Don't need it...

	if (InputConnected(m_audioGrabberBF)) m_hasaudio = true;
	else m_audioGrabberBF.Release();

	if (m_hasvideo)
	{
		// Replace downstream of video and audio sample grabbers with a null renderer
		if (!NullRendererDownstream(m_videoGrabberBF)) return false;
	...
	if (m_hasaudio)
	{
		if (!NullRendererDownstream(m_audioGrabberBF)) return false;

		audioGrabber->SetBufferSamples(false);

		if (!m_ignoreAudio) audioGrabber->SetCallback(m_audioCallback, 0);
		else m_hasaudio = false;
...
bool CMovieSource::NullRendererDownstream(IBaseFilter* filter)
{
	...
	if (!DeleteDownstream(filter)) return false;
	...
	if (FAILED(nullRenderer.CoCreateInstance(CLSID_NullRenderer))) return Error(TEXT("Unable to create null renderer filter"));

	if (FAILED(m_graphBuilder->AddFilter(nullRenderer, L"NullRenderer"))) return Error(TEXT("Unable to add null renderer to the filter graph"));
	...
	if (FAILED(GetPin(nullRenderer, PINDIR_INPUT, 0, pNullIn))) return Error(TEXT("Unable find the null renderer input pin."));

	if (FAILED(m_graphBuilder->Connect( pGrabOut, pNullIn ))) return Error(TEXT("Unable find the sample grabber output pin."));
...
bool CMovieSource::DeleteDownstream(IBaseFilter* filter)
{
	...
	CComPtr<IPin> pGrabOut;
	if (GetPin(filter, PINDIR_OUTPUT, 0, pGrabOut) != S_OK || pGrabOut == NULL) return true; // If no output pin

	// What is it connected to?
	CComPtr<IPin> pGrabToPin;
	hr = pGrabOut->ConnectedTo(&pGrabToPin);
	if (pGrabToPin == NULL) return true; // If output pin not connected to anything
...
bool CMovieSource::ReadImage(CGrImage& p_image)
{
	...
	if (!m_hasvideo) return false;

	if (m_videoeof) return false;
	...
	if (s.m_end >= m_duration) m_videoeof = true;
...
bool CMovieSource::ReadAudio(std::vector<short>& p_audio)
{
	...
	while (true)
	{
		...
		if (m_videoframeduration > 0) desiredendtime = m_videoposition + m_videoframeduration + 0.001; // Will accept till this time
		else desiredendtime = m_videoposition + 0.035; // Will accept till this time
	...
	if (m_videoframerate > 0)
	{
		...
		if (framesavailable > samplesperframe) framesavailable = samplesperframe + 1;
	}
	...
	if (m_audioposition >= m_duration) m_audioeof = true;
...
bool CMovieSource::SeekTime(double p_time)
{
	...
	{
		if (cap & AM_SEEKING_CanSeekAbsolute) canseek = true;
	}
...
//! \cond
ULONG CMovieSourceCallback::AddRef()
{
	return 2;
}

ULONG CMovieSourceCallback::Release()
{
	return 1;
}
...
{
	if (m_isVideo) m_user->NewVideoSample(pSample);
	else m_user->NewAudioSample(pSample);
	return 0;
};

HRESULT CMovieSourceCallback::BufferCB(double dblSampleTime, BYTE* pBuffer, long iBufSize)
{
	return 0;
}
