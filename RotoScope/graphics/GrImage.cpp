...
int CGrImage::GetPlanes() const
{
	return m_planes;
}

int CGrImage::GetRowPitch() const
{
	return m_rowpitch;
}
...
bool CGrImage::IsEmpty() const
{
	return m_width <= 0 || m_height <= 0;
}

BYTE* CGrImage::operator[](int i)
{
	return m_image[i];
}

const BYTE* CGrImage::operator[](int i) const
{
	return m_image[i];
}

BYTE* CGrImage::GetRow(int i)
{
	return m_image[i];
}

const BYTE* CGrImage::GetRow(int i) const
{
	return m_image[i];
}

int CGrImage::GetWidth() const
{
	return m_width;
}

int CGrImage::GetHeight() const
{
	return m_height;
}

BYTE* CGrImage::GetImageBits() const
{
	return m_image[0];
}
...
{
	if (planes <= 0) SetSize(p_img.m_width, p_img.m_height, p_img.m_planes);
	else SetSize(p_img.m_width, p_img.m_height, planes);
}
...
void CGrImage::SetSize(int p_x, int p_y, int planes)
{
	...
	if (p_x == m_width && m_height == p_y && m_planes == planes) return;
